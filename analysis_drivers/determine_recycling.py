...
original_outflow_rate = OR.outflow_flux_atshell(G['m'][Gclose][Ghalo], v_rad, OutflowCut, ShellCuts[index1], ShellThickness_ar[2], h)
...
Particles_G2_095 = np.copy(G['id'][Gclose][Ghalo][ShellCutsInner])

print 'length of recyled outflowing particles',len(OutflowingParticlesID_G2_095Rvir)
len95  = len(Particles_G2_095)
...
#number of particles within shell#2 epoch2 that are from burst1 epoch1
...
#number of partciles from shell1 epoch1 that are back within shell2
here_backfromburst  =  np.in1d(OutflowingParticlesID_025Rvir, Particles_G2_095)
len95backfrom25 =  len(OutflowingParticlesID_025Rvir[here_backfromburst])

print halo_to_do, Nsnap, Nsnap2, redshift1, redshift2, len25, len95, len95stillin25, len95backfrom25, (float(len95stillin25)/float(len95)), (float(len95backfrom25)/float(len25)),  index1, index2, use_v0

BigString = [halo_to_do, Nsnap, Nsnap2, redshift1, redshift2, len25, len95, len95stillin25, len95backfrom25, (float(len95stillin25)/float(len95)), (float(len95backfrom25)/float(len25)),  index1, index2, int(use_v0)]
thestring = SF.line_to_string(BigString)

foutname = 'recycle_outflow_burst'+str(Nsnap)+'halo'+str(halo_to_do)+'.txt'
