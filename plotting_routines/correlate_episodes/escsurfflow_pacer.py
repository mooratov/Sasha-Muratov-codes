...
if (len(sys.argv) < 2):
	print 'syntax  blah.py list_of_files.txt'
	sys.exit()
...
for name in finname_ar[1:]:
	...
	if not '#' in name:
		...
		maxweightedsig_ar.append(maxweightsig)
...
f = open(foutname, 'a')
color_ar = ['k', 'r', 'b', 'g', 'm', 'DarkGoldenRod', 'DarkSlateBlue', 'LightGreen']
...
for mass in mass_ar:
	...
	print vesc_ar[count][vesc_order]
	...
	myline = [count, lasta_ar[count], Neps_ar[count], weight_avg_vesc, vesc_25, vesc_med, vesc_75, weighted_avg_sig, unweighted_avg_sig, maxSF_ar[count], totMass_ar[count], duration_ar[count] ]
	...
	plt.plot(histx,histy, color=color_ar[count], ls = ls_ar[count], lw=2, alpha = 0.5)
	count+=1
